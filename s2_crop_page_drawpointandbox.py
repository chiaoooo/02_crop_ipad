...
def scaleAdjustment(word_img, adjustCentroid=True):
    ...
    #畫點
    #red point 質心
    word_img_copy = cv2.rectangle(word_img_copy, (cX-1, cY-1), (cX+1, cY+1), (0, 0, 255), 2)
    #green point 重心
    word_img_copy = cv2.rectangle(word_img_copy, (cX2-1, cY2-1), (cX2+1, cY2+1), (0, 255, 0), 2)
    #畫框框
...
def setPointImageFromPath(args) -> str:
    ...
    for j in range(10):
        ...
        for k in range(10):  
            ...
            # if show:
            #     #用藍色標出各個綠色框框
            #     cv2.rectangle(show_image, (result[0][0], result[0][1]), (result[1][0], result[1][1]), (0, 255, 255), 5)
            #     cv2.imshow('Image',
            #                cv2.resize(show_image,
            #                           (show_image.shape[1] // 8, show_image.shape[0] // 8),
            #                           interpolation=cv2.INTER_AREA)
            ...
            #畫框框       
                ...
                if j == 9 and k == 9 :
                    filename = f'{save_path}{now_page}{file_extension}'
            ...
            #     #finalWordImg, max_length = scaleAdjustment(word_img, adjustCentroid=adjustCentroid)
